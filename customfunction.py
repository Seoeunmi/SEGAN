...
def create_signal(original_signal, up_size):
    l = len(original_signal)
    upsample = [0] * l * up_size
    for i in range(l):
        upsample[i*up_size] = original_signal[i]
    return upsample
...
def window(window_name, frame_size):
    if window_name == 'hanning':
        ...
        raise Exception('Select hanning or hamming or sine')
